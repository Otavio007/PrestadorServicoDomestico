...
export default function ProviderChatDetailScreen() {
    ...
    async function loadData() {
        ...
        // Fetch Client Name and Messages in parallel for faster load
        fetchMessages(id);
        ...
        const { data: imgData } = await supabase
            ...
            .maybeSingle();
        if (imgData) setClientImage(imgData.img);
    }

    async function fetchMessages(providerId: string) {
        ...
        if (data) {
            setMessages(data.map((m: any) => ({
                ...m,
                sent_by_me: m.enviado_por === 'prestador'
            })));
        }
    }
    ...
    function subscribeToMessages(currentUserId: string) {
        return supabase
            ...
            .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'mensagem' }, (payload) => {
                ...
                if (newMessage.id_cliente === clientId &&
                    newMessage.id_prestador === currentUserId &&
                    newMessage.enviado_por !== 'prestador') {
                    setMessages(prev => {
        ...
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.header}>
                ...
                <View style={styles.headerInfo}>
                    <Image
                        source={{ uri: clientImage || `https://ui-avatars.com/api/?name=${clientName}&background=random` }}
                        style={styles.headerAvatar}
                    />
                    <Text style={styles.providerName}>{clientName}</Text>
                </View>
            </View>
